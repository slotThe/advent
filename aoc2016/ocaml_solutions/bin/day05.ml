...
let inp = read_single_line "../inputs/day05.txt"

let generate_hashes s =
  Seq.filter_map
    (fun i ->
      let hash = Md5.to_hex @@ Md5.digest_string @@ s ^ string_of_int i in
      if String.is_prefix hash ~prefix:"00000" then Some hash else None)
...
let part1 =
  Seq.fold_left
    (fun a h -> a ^ Char.to_string h.[5])
    ""
    (test_hash 8 inp (const true))
...
let () = print_answer 5 (part1, part2)
