...
let p_line s =
  let parts = String.split_on_chars s ~on:['['; ']']
  in List.partition_map (List.zip_exn parts (List.range 0 (List.length parts)))
       ~f:(fun (a, i) ->
         let a' = String.to_list a
         in if i mod 2 = 0 then First a' else Second a')

let find_abba xs =
  List.exists (sliding_window 4 xs)
    ~f:(function | [a; b; c; d] -> Char.(a = d && b = c && a <> b)
                 | _            -> false)

let part1 (supernet, hypernet) =
  List.exists supernet ~f:find_abba
  && not (List.exists hypernet ~f:find_abba)

let collect_bab xs =
  List.filter_map (sliding_window 3 xs)
