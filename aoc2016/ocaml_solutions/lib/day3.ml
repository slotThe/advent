...
let inp =
  ...
  in In_channel.read_all "../../inputs/day03.txt"
     |> String.split_lines
     |> List.map ~f:(fun s -> p_int s 0, p_int s 5, p_int s 10)

let could_be_triangle xs =
  let check (a, b, c) = a + b > c && a + c > b && b + c > a
  in List.length (List.filter xs ~f:check)
...
let part2 =
  ...
  in could_be_triangle
       List.(map ~f:tuple_to_list inp
             |> transpose_exn
             |> concat_map ~f:list_to_tuple)

let day3 = string_of_int (could_be_triangle inp)
         , string_of_int part2
